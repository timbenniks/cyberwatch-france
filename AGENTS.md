...
- **Theme follows the OS by default.** Light and dark are CSS custom properties under `prefers-color-scheme`. An explicit Light/Dark choice is stored in the `cw_theme` cookie and applied as `data-theme` on `<html>` by a blocking head script (same pattern as locale detect), so the first paint already matches. Do not store the theme in `localStorage`, and do not drive the selected control from SSR state — resolve it on the client. Charts read the resolved scheme via `useColorScheme()`.
...
- `npm run build` succeeds and prerenders 52 routes.
